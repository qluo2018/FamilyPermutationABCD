%% family relatedness given by combining the family info and the
% ABCD genetics to have the MZ, DZ, FS, and HS information that was not
% provided by the questionnaire acspsw02.txt in the ABCD study
...
%% input
% acspsw02.txt: ---- family qeustionnaire from ABCD release v1.0;
%                   (data for 4,521 subjects)
% kinship_0.0884.txt --- kinship matrix with kinship greater than 0.0884
%                        given by the KING package;
% familyinfo.mat --- family inforamtion extracted from acspsw02.txt:
%                    ID, famID,
%                    famRela(0--singleton, 1--sibling, 2--twin, 3--triplet)
%                    (data for 3,515 with both behaviour and neuroimaging
%                    data avaialbe at the baseline)
%          NOTE: 4 famID overlap between single and twin,
%                [584,596,2205,4454], we had re-set those 4 singleton
%                [famID==?? & famRela_final==0] to sibling (1).
% ABCD_release_2.0.1.csv  --- subject ID with gene data releaseed by
%                             ABCD v2.0.1  (10,627 subjects)
%% output
% imagingInfo2.mat ---- c for subject ID;
%                       famID_1 for family ID;
%                       famRela for family relatioinship from the
%                               questionannire;
%                       sibtype for sibling type;
%                       famtype for family type
...
% LPC2, 3BZ3, M3Y9 --> 'NS'
% 88CW and X4DY, 7LW2 and 0W26, 5895 and 4CGV, ML75 and COU4,
% 85TU and BEZ6, R9C4 and 9HZA, KCYL and G6BN --> 'FS'
% HNGB and WY31 --> 'DZ' (ages were the same)
% RV36 and YJ7F, 7T9X and KA9M, HRKP and 6461, RD6E and LPZT, D51D
% and X4EW --> 'HS'
...
for j = 1 : size(kinship0884,1)
    ...
    if sum(strcmp(imagingInfo.c,kinship0884.ID1{j})) ...
            && sum(strcmp(imagingInfo.c,kinship0884.ID2{j}))
        withinIndex(j) = 1;
    end
end
disp(toc);
...
for i = 1 : length(imagingInfo.c)
    ...
    kinship{i} = [];
    idkin = find(strcmp(kinship0884_within.ID1, imagingInfo.c{i}) ...
        + strcmp(kinship0884_within.ID2, imagingInfo.c{i}));
...
% end
% table(ID(idnow), age(idnow), famID(idnow), 'VariableNames', ...
% {'ID', 'age', 'fID'})
...
for i = 1 : length(imagingInfo.c)
    ...
    if maxkin(i) >= 0.354
        ...
        if famRela(idnow) <= 1
            count(1) = count(1) + 1;
            disp(['inconsistent: MZ twin ', imagingInfo.c{i}, ...
                '; famID: ', num2str(famID(idnow))]);
            idkin = find(strcmp(kinship0884_within.ID1, ...
                imagingInfo.c{i}) + strcmp(kinship0884_within.ID2, ...
                imagingInfo.c{i}));
    ...
    elseif maxkin(i) >= 0.177
        ...
        elseif famRela(idnow) == 1
            ...
            count(2) = count(2) + 1;
            disp(['inconsistent: DZ or FS ', imagingInfo.c{i}, ...
                '; famID: ',  num2str(famID(idnow))]);
            
            idkin = find(strcmp(kinship0884_within.ID1, ...
                imagingInfo.c{i}) + strcmp(kinship0884_within.ID2, imagingInfo.c{i}));
            if ~isempty(setdiff(idkin, record_all))
                record_all = [record_all; idkin];
            end
        end
    elseif maxkin(i) >= 0.0884
        ...
        if famRela(idnow) == 1
            ...
            count(3) = count(3) + 1;
            disp(['inconsistent: HS ', imagingInfo.c{i}, '; famID: ', ...
                num2str(famID(idnow))]);
            idkin = find(strcmp(kinship0884_within.ID1, ...
                imagingInfo.c{i}) + strcmp(kinship0884_within.ID2, imagingInfo.c{i}));
            if ~isempty(setdiff(idkin, record_all))
...
% LPC2 and D3H6 shared a kinship as 0.4988, but they were at different ages
% (128 and 124, respectively) and with different familyID
...
% 3U3X and 3BZ3 shared a kinship as 0.4898, but they were at different ages
% (123 and 108, respectively) and with different familyID


%% delete 3U3X and JH41 (according to the questionnaire they were
% twins, but JH41 did not have genetic data,
% so we could not determin MZ and DZ)
deleteSet = [deleteSet, find(strcmp(imagingInfo.c, 'NDAR_INVTHEX3U3X')), ...
    find(strcmp(imagingInfo.c, 'NDAR_INVNNA7JH41'))];
%find(strcmp(genetics.Var2, 'NDAR_INVNNA7JH41'))
sibtypeName(strcmp(imagingInfo.c, 'NDAR_INVBGMH3BZ3')==1,:) = 'NS';

%% deal with count(2) --> 16 : kinship was higher than 0.177
% but family info was not twin or triplets or sibling
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVK1CF88CW')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVJZ6BX4DY')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVFRT17LW2')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVWZCU0W26')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV3YRA5895')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV7LB64CGV')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVJVWCML75')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVK0LXC0U4')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV1K1285TU')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV1E35BEZ6')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV6YFHHNGB')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVGALZWY31')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV08K0R9C4')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVEJ0L9HZA')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV6R35KCYL')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV22E7G6BN')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVUDUKRV36')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV87WRYJ7F')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVGBLP7T9X')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV0RHLKA9M')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV3ZM8HRKP')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV93AY6461')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVN0BDRD6E')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVPM21LPZT')==1);
...
imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INV23XZD51D')==1) ...
    = imagingInfo.famID_1(strcmp(imagingInfo.c, 'NDAR_INVBDMDX4EW')==1);

%% 81 subjects had siblings or twins as specified in the
%  questionnaire but did not have the kinship estimation owing to either
%% no gene data of their owns (40) or no gene data of their
% relatives (41)
%% among them 34--registerred with siblins,
% 47--registerred with twins
...
%% only one from each family remain: the one with rel_group_id
% == 2 was deleted
...
for i = 1 : length(fidnow)
    ...
    if length(setnow) == 2
        deleteSet = [deleteSet, twid(setnow(2))]; % delete one of the twin
        %whose kinship could not be estimated owing to missing genetic data
    end
end
...
deleteSet = [deleteSet, find(strcmp(imagingInfo.c,  ...
    'NDAR_INVJ67B8F0J')),  find(strcmp(imagingInfo.c,  ...
    'NDAR_INVLBELG3A6')), find(strcmp(imagingInfo.c,  'NDAR_INVNDEK4K38'))];
...
imagingInfo2.famID_1 =...
    imagingInfo.famID_1(setdiff(1:length(imagingInfo.c),deleteSet),1);
